...
async function getSimulatorProjectFolders(): Promise<string[]> {
	const config = await configManager.load();
...
async function setSimulatorProjectFolders(
	folders: string[]
): Promise<string[]> {
...
async function pickSimulatorProjectFolder(): Promise<string | null> {
	try {
...
async function autoDetectSimulatorProjectFolders(): Promise<string[]> {
	const config = await configManager.load();
	const configured = Array.isArray(config.search_folders)
		? config.search_folders.filter(isString)
		: [];
...
async function getBaguetteStatus(): Promise<BaguetteStatus> {
...
async function startBaguetteServer(): Promise<BaguetteStatus> {
...
async function listSimulatorProjects(): Promise<SimulatorProject[]> {
...
async function openXcodeProject(appPath: string): Promise<string | null> {
...
async function buildInstallLaunchProject({
	appPath,
	udid,
	scheme,
}: {
