...
const STATUS_COLORS: Record<string, string> = {
	...
	"?": "text-inferay-muted-gray bg-inferay-white/5",
...
function StatusBadge({ status }: { status: string }) {
	const color =
		STATUS_COLORS[status] || "text-inferay-muted-gray bg-inferay-white/5";
	return (
		<span
...
function FileRow({
	...
	return (
		<button
			type="button"
			onClick={onClick}
			className="w-full flex items-center gap-1.5 px-2 py-1 text-left transition-colors hover:bg-inferay-dark-gray rounded-md group"
			title={file.path}
		>
			<StatusBadge status={file.status} />
			<span className="truncate text-[10px] text-inferay-white flex-1">
				{filename}
			</span>
			{dir && (
				<span className="text-[9px] text-inferay-muted-gray truncate max-w-[80px]">
					{dir}
				</span>
			)}
		</button>
...
function ProjectSection({
	...
	return (
		<div className="border-b border-inferay-gray-border/50 last:border-b-0">
			<button
				type="button"
				onClick={() => setExpanded(!expanded)}
				className="w-full flex items-center gap-2 px-2 py-1.5 text-left transition-colors hover:bg-inferay-dark-gray/50"
			>
				<IconChevronRight
					size={8}
					className={`shrink-0 text-inferay-muted-gray transition-transform ${expanded ? "rotate-90" : ""}`}
				/>
				<span className="text-[11px] font-medium text-inferay-white truncate flex-1">
					{project.name}
				</span>
				...
				{totalChanges > 0 && (
					<span className="text-[9px] font-semibold text-inferay-muted-gray tabular-nums">
						{totalChanges}
					</span>
				)}
			</button>
			...
			{expanded && (
				<div className="pb-1">
					...
					{untrackedFiles.length > 0 && (
						<div className="px-1">
							<div className="px-2 py-0.5 text-[9px] font-semibold text-inferay-muted-gray/70 uppercase tracking-wider">
					...
					{totalChanges === 0 && (
						<p className="px-3 py-2 text-[10px] text-inferay-muted-gray">
							Clean working tree
						</p>
					)}
				</div>
...
export const GitSidebar = memo(function GitSidebar({
	...
	return (
		<CollapsibleSidebarSection
			...
			count={totalChanges}
			countColor={
				totalChanges > 0 ? "text-yellow-400" : "text-inferay-muted-gray"
			}
			expanded={expanded}
			onToggle={onToggle}
